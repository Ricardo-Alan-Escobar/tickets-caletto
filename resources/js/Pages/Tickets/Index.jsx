...
export default function TicketsIndex({ auth, tickets }) {
    ...
    const { data, setData, delete: destroy, post, put, processing, reset, errors } = useForm({
        Nombre: '', Departamento:'', Edificio:'', Problema: '', Prioridad: '', Estado: '',  Creacion: '', Termino: ''
    });

    const openModal = (op, id = '', Nombre= '', Departamento='', Edificio='', Problema= '', Prioridad= '', Estado= '',  Creacion= '', Termino= '') => {
        setModal(true);
        setOperation(op);
        if (op === 1) {
            setTitle('Añadir Ticket');
            setData({ Nombre: '', Departamento:'', Edificio:'', Problema: '', Prioridad: '', Estado: '',  Creacion: '', Termino: ''});
        } else {
            setTitle('Editar Ticket');
            setData({ id, Nombre, Departamento, Edificio, Problema, Prioridad, Estado,  Creacion, Termino});
        }
    }
        ...
        >
            ...
            <div className="grid min-h-screen place-items-center bg-gray-100 p-4">
                ...
                <div className="flex justify-between w-full mb-2">
                    ...
                    <PrimaryButton onClick={() => openModal(1)} className="ml-4 w-1/4 text-center">
                    <FontAwesomeIcon icon={faUserPlus} className="ml-16 pr-1" /> 
                        Añadir Ticket
                    </PrimaryButton>
                </div>
                <div className="w-full overflow-hidden bg-white rounded-lg shadow-md ">
                    <table className="min-w-full leading-normal">
                        <thead>
                            <tr className="bg-blue-600 text-white">
                                ...
                                <th className="px-4 py-2 text-center">Edificio</th>
                        ...
                        <tbody>
                            {currentTickets.map((ticket, i) => (
                                <tr key={ticket.id} className={i % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                                    ...
                                    <td className="px-4 py-2 text-center">{ticket.Edificio}</td>
                                    ...
                                    <td className="px-4 py-2 text-center">
                                        <WarningButton onClick={() => openModal(2, ticket.id, ticket.Nombre, ticket.Departamento, ticket.Edificio, ticket.Problema, ticket.Prioridad, ticket.Estado, ticket.Creacion, ticket.Termino)} className="mr-2">
                    ...
                    <div className="flex justify-center mt-2 mb-2">
                        <ReactPaginate
                           previousLabel={<button className="px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-500"><FontAwesomeIcon icon={faArrowLeft} /></button>}
                            nextLabel={<button className="px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-500"><FontAwesomeIcon icon={faArrowRight} /></button>}
            ...
            <Modal show={modal} onClose={closeModal}>
                ...
                <form onSubmit={save} className="p-0 mr-4 ml-4">
                    ...
                    <div className='mt-2'>
                        <InputLabel htmlFor="Edificio" value="Edificio"></InputLabel>
                        <SelectInput
                            id="Edificio"
                            name="Edificio"
                            ref={edificioInput}
                            ...
                            handleChange={(e) => setData('Edificio', e.target.value)}
                            options={['Edificio 1', 'Edificio 2']}
                            required
                        />
